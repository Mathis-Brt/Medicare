...
if ($db_handle) {
    ...
    $sql = "SELECT * FROM medecinspe WHERE spécialité = 'IST'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        echo "<a href='#' style='color: white;'>IST</a>";
        echo "</div>";

        // Requête SQL pour récupérer l'e-mail du médecin
        $sql_email = "SELECT mail FROM medecinspe WHERE spécialité = 'IST' LIMIT 1"; // Limitez à 1 car c'est pour un seul médecin
        ...
        <div class="doctor-container">
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<p><strong>Médecin IST:</strong> Dr " . htmlspecialchars($row['nom']) . "</p>";
